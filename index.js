...
app.post('/api/register', upload.single('file'), async (req, res) => {
  try {
    ...
    // Read and upload to IPFS
    const filePath = path.join(__dirname, 'uploads', req.file.filename);
    console.log("=== FILE BUFFER DEBUG ===");
    console.log(fs.readFileSync(filePath).toString().slice(0, 200));

    const fileBuffer = fs.readFileSync(filePath);
    // const result = await ipfs.add(fileBuffer);

    // Detect if file is text or wrongly encoded bytes
    let dataToUpload;
    try {
    // Try decoding buffer as UTF-8 text first
    const text = fileBuffer.toString('utf8');
    // If it contains only digits and commas, it's likely numeric ASCII codes -> convert back
    ...
    // Register function in smart contract
    let txHash;
    try {
      const accounts = await web3.eth.getAccounts();
      const receipt = await contract.methods.registerFunction(cid, req.body.name).send({ from: accounts[0], gas: 500000  });
